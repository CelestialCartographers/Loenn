...
local circleStruct = require("structs.circle")
...
local function getDragRadius()
    return utils.round(math.sqrt((startX - dragX)^2 + (startY - dragY)^2))
end
...
local function handleDragFinished()
    ...
    if room and startX and startY and dragX and dragY then
        ...
        local radius = getDragRadius()
        local brushStartX, brushStartY = math.floor(startX - radius), math.floor(startY - radius)
        local brushStopX, brushStopY = math.ceil(startX + radius), math.ceil(startY + radius)
...
local function updatePoints()
    if startX and startY and dragX and dragY then
        local radius = getDragRadius()
        local circle = circleStruct.create(startX, startY, radius)

        points = circle:getPoints(tool.mode)
